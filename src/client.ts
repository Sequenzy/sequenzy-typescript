...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['SEQUENZY_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SEQUENZY_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Sequenzy API.
 */
export class Sequenzy {
  apiKey: string;
   ...
   * API Client for interfacing with the Sequenzy API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['SEQUENZY_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['SEQUENZY_BASE_URL'] ?? https://api.sequenzy.com/api/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('SEQUENZY_BASE_URL'),
    apiKey = readEnv('SEQUENZY_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.SequenzyError(
        "The SEQUENZY_API_KEY environment variable is missing or empty; either provide it, or instantiate the Sequenzy client with an apiKey option, like new Sequenzy({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.sequenzy.com/api/v1`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Sequenzy.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SEQUENZY_LOG'), "process.env['SEQUENZY_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('SEQUENZY_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.sequenzy.com/api/v1';
  }
  ...
  static Sequenzy = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static SequenzyError = Errors.SequenzyError;
